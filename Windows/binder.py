...
OUTPUT_FILE = "bound"
COMPILER = "C:\Users\AustinVM\Downloads\mingw32\\bin\g++.exe"
...
def compile_header_file(o_file):
	...
	process = Popen([COMPILER, "binderbackend.cpp", "-o", o_file, "-std=gnu++11"], stdout=PIPE)
...
def hexdump(i_file):
	...
	with open(i_file, 'rb') as f:
		...
		# Convert the hex to 0x00 format
		hex_converted = ['0x' + hex_data[i:i+2].upper() for i in range(0, len(data), 2)]
...
def main():
	...
	for program in argv:
		...
		prog_output = hexdump(program)

		# Get number of bits of hex output
		num_bits = len(prog_output) # counting number of commas, so account for extra bit at the end

		# Add the num_bits to an array for later
		program_lengths.append(num_bits)

		# Dump hex into header file
		code_array_file.write("  new unsigned char[" + str(num_bits) + "] {" + ','.join(prog_output) + "},")
